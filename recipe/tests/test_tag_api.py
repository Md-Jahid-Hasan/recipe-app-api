from django.contrib.auth import get_user_model
from django.urls import reverse
from django.test import TestCase

from rest_framework import status
from rest_framework.test import APIClient

from core.models import Tag

from recipe.serializers import TagSerializers

TAG_URLS = reverse('recipe:tag-list')


class PublicTagsApiTests(TestCase):
    """Test the publicly available test Api"""

    def setUp(self):
        self.client = APIClient()

    def test_login_required(self):
        """Test the login is required for retriving Tag"""
        res = self.client.get(TAG_URLS)

        self.assertEqual(res.status_code,status.HTTP_401_UNAUTHORIZED)


class PrivateTagApiTest(TestCase):
    """Test the authorized User API"""

    def setUp(self):
        self.user = get_user_model().objects.create_manager(
            "test@gmail.com",
            "pass123"
        )
        self.client = APIClient()
        self.client.force_authenticate(self.user)

    def test_retrive_tags(self):
        """Test retriving tags"""
        Tag.objects.create(user=self.user, name="Binty")
        Tag.objects.create(user=self.user, name="Jahid")

        res = self.client.get(TAG_URLS)

        tags = Tag.objects.all()
        serializer = TagSerializers(tags, many=True)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(res.data, serializer.data)

    def test_tags_limited_to_user(self):
        """test that tags return are for authenticate user"""
        user2 = get_user_model().objects.create_manager(
            "another@gmail.com",
            "test123",
        )
        Tag.objects.create(user=user2, name='Mango')
        tag = Tag.objects.create(user=self.user, name="Foodie")

        res = self.client.get(TAG_URLS)

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(len(res.data), 1)
        self.assertEqual(res.data[0]['name'], tag.name)
